...
if (file.exists('activity.csv')){}else{
...
We are goign to use data.tables to replace all NA interval figures  

with the interval average figures from before - [source link to stack overflow](http://stackoverflow.com/questions/7971513/using-one-data-frame-to-update-another)  
...
hist(newdaysum$stepcount , main = "histogram of daily stepcount", xlab = "stepcount")
summary(newdaysum$stepcount)
```
So we can see that there were `r part1` NA rows in the data set and perhaps  
not surprisingly replacing them with our average hasnt changed much in the histogram
